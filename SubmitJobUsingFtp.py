...
# sourceFilePath = r'D:\gowrishankar.p\IMS to DB2 Conversion Tool\OneDrive_1_12-6-2021\IMS.PSB.SOURCE'
sourceFilePath = r'D:\gowrishankar.p\DB2 to SQL Migrator tool'
# targetFolderPath = 'G176281.IMS.PSB.SOURCE'
targetFolderPath = 'G176281.DB2.FTP.JCLLIB'
...
with open(scriptPath, "w") as script:
    ...
    for Member in sourceFile:
        # mem = mem.replace('.txt','')
        # script.write('put -transfer=ascii "{}"\n'.format(mem))
        if Member == 'Unload_Script.txt':
            mem = Member.replace('_Script.txt','')
            script.write('put -transfer=ascii {}'.format(Member) + ' '+mem+'\n')
    # mem = 'UNLOAD3'
    # script.write('put -transfer=ascii "{}"\n'.format(mem))
    #script.write("call site filetype=jes JESJOBNAME=*\n")
    script.write("exit")
