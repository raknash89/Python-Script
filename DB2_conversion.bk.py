...
##    configd['MFUSRID'] =  MFUSRID
    ...
    #MFJOBLIB = input ("Enter the Mainframe PDS where you wish to execute the jcls from if its different from FTP directory specified above:").upper()
    #if MFJOBLIB != '':
    #    configd['MFJOBLIB'] = MFJOBLIB
    #else:
    #    MFJOBLIB = MFFTPIN
    #    configd['MFJOBLIB'] = MFJOBLIB
...
##  Re-direct from FTPLIB connect to WINSCP FTP    
    ...
    for idx, line in enumerate(skeleton):      
        if line[:10] == "//JOBNAMEX":
            ...
            for tab in table:
                ...
                # REPEAT DISP PARAMETER USING INDEX
##                if count < len(table*2) and count != len(table):
##                    load.append(skeleton[idx+1])
##                    load.append(skeleton[idx+2])
##                    
        elif 11 <= idx <=28:
...
def Inventory(skeleton,opt):
    global data1
    with open("Inventory_test.csv", "r") as b:
